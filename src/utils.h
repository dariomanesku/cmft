...
{
#if defined(_MSC_VER)
    static float fminf(float _a, float _b)
    {
        return _a < _b ? _a : _b;
    }

    static float fmaxf(float _a, float _b)
    {
        return _a > _b ? _a : _b;
    }

    // TODO: use c++11 log2 from <cmath.h>
    static float log2f(float _val)
    {
    ...
    /// Assumes _min < _max.
    template <typename T>
    static T clamp(T _val, T _min, T _max)
    {
    ...
    static void cmft_swap(uint8_t* _a, uint8_t* _b, uint32_t _size)
    {
    ...
    static uint32_t alignf(float _val, uint32_t _align)
    {
        return uint32_t(_val/float(int32_t(_align)))*_align;
    }

    static uint32_t align(uint32_t _val, uint32_t _alignPow2)
    {
    ...
    static void strtolower(char* _out, char* _in)
    {
        for( ; *_in; ++_in)
        {
            *_out++ = tolower(*_in);
    ...
    static void strtolower(char* _str)
    {
        for( ; *_str; ++_str)
        {
            *_str = tolower(*_str);
    ...
    static void strtoupper(char* _out, char* _in)
    {
        for( ; *_in; ++_in)
        {
            *_out++ = toupper(*_in);
    ...
    static void strtoupper(char* _str)
    {
        for( ; *_str; ++_str)
        {
            *_str = toupper(*_str);
    ...
    static void cmft_strncpy(char* _dst, const char* _src, size_t _srcLen)
    {
        if (NULL != _src)
        {
            _dst[0] = '\0';
    ...
    ///     /tmp/foo.c -> foo
    ///     C:\\tmp\\foo.c -> foo
    static bool getFileName(char* _fileName, size_t _maxLenght, const char* _filePath)
    {
       const char *begin;
       const char *end;
